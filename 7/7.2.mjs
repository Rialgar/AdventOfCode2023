...
function rateHand(hand) {
    ...
    const maxCounters = counters.slice(1).filter(c => c>0).sort((a,b) => b-a);
    if(maxCounters[0] + jokers === 5 || jokers === 5){
        rating += 6 * WEIGHTS[HAND_SIZE];
    } else if(maxCounters[0] + jokers === 4){
        rating += 5 * WEIGHTS[HAND_SIZE];
    } else if(maxCounters[0] + jokers === 3 && maxCounters[1] === 2){
        rating += 4 * WEIGHTS[HAND_SIZE];
    } else if(maxCounters[0] + jokers === 3){
        rating += 3 * WEIGHTS[HAND_SIZE];
    } else if(maxCounters[0] + jokers === 2 && maxCounters[1] === 2){
        rating += 2 * WEIGHTS[HAND_SIZE];
    } else if(maxCounters[0] + jokers === 2){
